...
class PathfinderTab(QWidget):
    ...
    def build_ui(self):
        ...
        self.zero_yaw_btn.clicked.connect(self.send_zero_yaw)
    ...
    def handle_serial_line(self, line):
        ...
        if tag == "ZERO_YAW_ACK":
            if len(parts) >= 2:
                self.last_zero_yaw_ack = parts[1].upper()
                self.status_label.setText("Status: Zero yaw stored")
            return
    ...
    def send_zero_yaw(self):
        if self.shared_serial_host is None and self.bt_thread is None:
            QMessageBox.warning(self, "Not Connected", "Connect Bluetooth before sending ZERO_YAW.")
            return

        self.last_zero_yaw_ack = None
        self.send_command_with_delay("ZERO_YAW")
        self.wait_for_ack("last_zero_yaw_ack", "OK")
