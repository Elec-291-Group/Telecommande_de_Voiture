...
// Called every 600 µs (one SIRC T unit).
// State machine:
//   FSM_IDLE    — wait for fsm_trigger, then start frame
//   FSM_SENDING — step through sirc_units[], pad silence to 75 units (45 ms)
void Timer2_ISR(void) __interrupt (5)
{
	...
	if (fsm_state == FSM_SENDING)
	{
		...
		// Return to idle after the full 45 ms period (75 T units)
